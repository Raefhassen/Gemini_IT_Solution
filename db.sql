CREATE TABLE `gemini_its_amm_db`.`its_roles` (
  `id` CHAR(36) NOT NULL,
...
DROP TRIGGER IF EXISTS `gemini_its_amm_db`.`role_uniq_id`;
...
CREATE TABLE `gemini_its_amm_db`.`its_users` (
	...
	`Locked` TINYINT(1) NULLDEFAULT 0,
	PRIMARY KEY (`id`),  
	UNIQUE INDEX `id_UNIQUE` (`id` ASC),  
...
ALTER TABLE `gemini_its_amm_db`.`its_users` ADD CONSTRAINT `AppRole`   FOREIGN KEY (`Role`)   
REFERENCES `gemini_its_amm_db`.`its_roles` (`Role`)   ON DELETE NO ACTION   ON UPDATE NO ACTION;

DROP TRIGGER IF EXISTS `gemini_its_amm_db`.`OI_new_user`;
DELIMITER $$
	USE `gemini_its_amm_db`$$
	CREATE DEFINER = CURRENT_USER TRIGGER `gemini_its_amm_db`.`OI_new_user` 
...
DROP TRIGGER IF EXISTS `gemini_its_amm_db`.`BU_users`;
DELIMITER $$
	USE `gemini_its_amm_db`$$
	CREATE DEFINER = CURRENT_USER TRIGGER `gemini_its_amm_db`.`BU_users` 
...
INSERT INTO `gemini_its_amm_db`.`its_users` (`FirstName`, `LastName`, `eMailAdresse`, `Login`, `Role`, `Passwordhash`) VALUES ('Administrator', 'Gemini Super User', 'Admin@gemini.com', 'Admin@gemini.com', 'SuperUser', 'Gemini@dmin1');
